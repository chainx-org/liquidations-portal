					...
					while (1) {
						switch (_context.prev = _context.next) {
							...
							case 2:
								...
								newArgs = [].concat(args, lodash.times(correctArgsLength - 1 - args.length, function () {
									return undefined;
								}), [options]);
...
var PROXY_REGISTRY$2 = "0x9dE275d0105EEa5e967083db98F0E9CE7acC5409";
var MULTICALL$2 = "0xe440AC03760696B8bCF0b64C607Bd92c71883E83";
var sherpaxAddresses = {
...
var networks = [{
	...
	name: 'sherpax',
	networkId: 1506,
	contracts: /*#__PURE__*/ contractInfo('sherpax')
}, {
	name: 'kovan',
	networkId: 42,
...
var currencies = /*#__PURE__*/ lodash.values(tokens).reduce(function (output, symbol) {
...
var GasService = /*#__PURE__*/ function (_PublicService) {
	...
	_proto._setProperties = function _setProperties(settings, label) {
		...
		return lodash.map(settings, function (value, key) {
			if (key === 'disable') {
...
var checkForErrors = function checkForErrors(values) {
	return lodash.find(values, function (v) {
...
var MulticallService = /*#__PURE__*/ function (_PublicService) {
	...
	_proto._watch = function _watch(_ref3, key) {
		...
		var existing = lodash.get(this._observables, obsPath);
		...
		if (computed) {
			...
			lodash.set(this._observables, obsPath, _observable);
		...
		lodash.set(this._subjects, obsPath, subject);
		...
		lodash.set(this._observables, obsPath, observable);
		return observable;
	};
	...
	_proto._addSchemaToMulticall = function _addSchemaToMulticall(schemaInstance) {
		...
		if ((_process = process) != null && _process.browser) log2('Active schemas (' + this._totalActiveSchemas + ' total):', this.activeSchemaIds);
		else log2("Active schemas (" + this._totalActiveSchemas + " total): " + this.activeSchemaIds.join(','));
	};
	...
	_proto._removeSchemaImmediately = function _removeSchemaImmediately(id) {
		...
		if (--this._totalActiveSchemas === 0) {
			...
			if ((_process2 = process) != null && _process2.browser) log2('Active schemas (' + this._totalActiveSchemas + ' total):', this.activeSchemaIds);
			else log2("Active schemas (" + this._totalActiveSchemas + " remaining): " + this.activeSchemaIds.join(','));
		}
	};
	...
	_proto._subscribeToWatcherUpdates = function _subscribeToWatcherUpdates() {
		...
		this._watcherUpdates = this._watcher.subscribe(function (update) {
			var subject = lodash.get(_this6._subjects, update.type);
...
var SmartContractService = /*#__PURE__*/ function (_PrivateService) {
	...
	_proto.getContractAddresses = function getContractAddresses() {
		return lodash.mapValues(this._getAllContractInfo(), function (versions) {
	...
	_proto._getAllContractInfo = function _getAllContractInfo() {
		...
		if (!this._contractInfoCache[networkName]) {
			...
			this._contractInfoCache[networkName] = lodash.mapValues(allContractInfo, function (versions, name) {
...
var Tracker = /*#__PURE__*/ function () {
	...
	_proto2.clearExpiredTransactions = function clearExpiredTransactions() {
		...
		lodash.each(this._transactions, function (txList, key) {
...
var Web3Service = /*#__PURE__*/ function (_PrivateService) {
	...
	_createClass(Web3Service, [{
		...
		key: "rpcUrl",
		get: function get() {
			var provider = lodash.last(this._web3.currentProvider._providers);
...
function mergeServiceConfig(role, sink, source, resolver) {
	...
	return [typeof source[0] != 'boolean' ? source[0] : sink[0], lodash.merge({}, sink[1], source[1])];
}
...
function createConfig(preset, options, resolver) {
	...
	if (config.web3) {
		...
		if (options.provider) {
			lodash.merge(web3Settings.provider, options.provider);
		}
...
function mergeOptions(object, source) {
	return lodash.mergeWith(object, source, function (objValue, srcValue, key) {
		if (Array.isArray(objValue) && key === 'abi') return lodash.uniq(objValue);
		if (Array.isArray(objValue) && key !== 'abi') return lodash.uniq(objValue.concat(srcValue));
	});
}
			...
			while (1) {
				switch (_context.prev = _context.next) {
					case 0:
						...
						userOptions = lodash.cloneDeep(otherOptions);
...
exports.utils = utils;
//# sourceMappingURL=dai.cjs.development.js.map